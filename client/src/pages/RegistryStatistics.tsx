...
function AggregateDonutChart({ title, description, data, total, selectedKey, onSelect }: { title: string; description: string; data: AggregateChartDatum[]; total: number; selectedKey: string; onSelect: (key: string) => void }) {
  return <Card className="border-[#e1e9e5] shadow-sm"><CardHeader className="pb-1"><CardTitle className="text-base text-[#24434b]">{title}</CardTitle><CardDescription>{description}</CardDescription></CardHeader><CardContent><ChartContainer config={chartConfig} className="mx-auto h-[230px] max-w-[310px]"><PieChart><ChartTooltip content={<ChartTooltipContent hideLabel formatter={(value) => `${value} records (${getStatisticShare(Number(value), total)}%)`} />} /><Pie data={data} dataKey="total" nameKey="label" innerRadius={59} outerRadius={92} paddingAngle={3} strokeWidth={0}>{data.map(item => <Cell key={item.key} fill={item.color} />)}</Pie><text x="50%" y="47%" textAnchor="middle" className="fill-[#1d3c46] text-2xl font-semibold">{total}</text><text x="50%" y="58%" textAnchor="middle" className="fill-[#68807e] text-[10px] font-medium">MATCHING RECORDS</text></PieChart></ChartContainer><div className="mt-2 grid gap-2 sm:grid-cols-2" aria-label={`${title} interactive filters`}>{data.map(item => <button key={item.key} type="button" onClick={() => onSelect(item.key)} aria-pressed={selectedKey === item.key} className={`flex min-h-10 items-center justify-between gap-2 rounded-lg border px-2.5 text-left text-xs transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#2d7a73] ${selectedKey === item.key ? "border-[#5fa397] bg-[#edf8f5] text-[#184a47]" : "border-[#e0ebe7] bg-white text-[#45616a] hover:bg-[#f4faf8]"}`}><span className="flex min-w-0 items-center gap-2"><span className="h-2.5 w-2.5 shrink-0 rounded-full" style={{ backgroundColor: item.color }} aria-hidden="true" /><span className="truncate">{item.label}</span></span><span className="font-semibold tabular-nums">{item.total} · {getStatisticShare(item.total, total)}%</span></button>)}</div><p className="mt-3 text-xs text-slate-500">Select a category below to apply its aggregate filter.</p></CardContent></Card>;
}

function AggregateBarChart({ title, data, total }: { title: string; data: AggregateChartDatum[]; total: number }) {
...
export default function RegistryStatistics() {
  ...
  const input = React.useMemo(() => ({ cohort: cohort === "all" ? undefined : cohort, completenessStatus: completeness === "all" ? undefined : completeness, enrollmentStatus: enrollment === "all" ? undefined : enrollment }), [cohort, completeness, enrollment]);
