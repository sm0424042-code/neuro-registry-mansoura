...
const cohorts = [
  ...
  { key: "epilepsy", label: "Epilepsy", color: "bg-[#8a6f3d]" },
] as const;
const brainVisual = "/manus-storage/mansoura-brain-imaging-overview_f9f978a6.png";
const fallbackBrainVisual = `data:image/svg+xml;charset=UTF-8,${encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 720" role="img" aria-labelledby="title desc"><title id="title">Mansoura Neurology visual fallback</title><desc id="desc">Abstract neural network fallback illustration. No patient image is used.</desc><defs><linearGradient id="bg" x1="0" x2="1" y1="0" y2="1"><stop stop-color="#061923"/><stop offset="1" stop-color="#123b43"/></linearGradient><radialGradient id="glow"><stop stop-color="#9ce7d8" stop-opacity=".5"/><stop offset="1" stop-color="#9ce7d8" stop-opacity="0"/></radialGradient></defs><rect width="1200" height="720" fill="url(#bg)"/><circle cx="820" cy="300" r="310" fill="url(#glow)"/><g fill="none" stroke="#87d9ca" stroke-opacity=".7" stroke-width="3"><path d="M650 170 780 250 910 185 1010 310 900 420 760 390 650 500"/><path d="M720 110 840 220 780 340 930 500 1050 400"/><path d="M630 330 760 390 820 570 970 540"/></g><g fill="#c2f4e9"><circle cx="650" cy="170" r="10"/><circle cx="780" cy="250" r="13"/><circle cx="910" cy="185" r="9"/><circle cx="1010" cy="310" r="11"/><circle cx="900" cy="420" r="14"/><circle cx="760" cy="390" r="10"/><circle cx="650" cy="500" r="9"/><circle cx="720" cy="110" r="8"/><circle cx="840" cy="220" r="10"/><circle cx="780" cy="340" r="11"/><circle cx="930" cy="500" r="9"/><circle cx="1050" cy="400" r="8"/><circle cx="630" cy="330" r="9"/><circle cx="820" cy="570" r="10"/><circle cx="970" cy="540" r="9"/></g><g fill="#d9f5ee" font-family="Arial, sans-serif"><text x="84" y="110" font-size="25" letter-spacing="5">MANSOURA UNIVERSITY · NEUROLOGY</text><text x="84" y="186" font-size="54" font-weight="700">Neural context remains available</text><text x="84" y="235" font-size="27" fill="#b8d9d2">Abstract fallback visual · no patient image</text></g></svg>`)}`;
...
export function HomeHeroMedia({ initialMode = "original", initialFailedRetryAttempts = 0, initialIsRetryLoading = false, onReportBrokenImage = () => undefined, reportState = "idle" }: { initialMode?: HomeHeroMediaMode; initialFailedRetryAttempts?: number; initialIsRetryLoading?: boolean; onReportBrokenImage?: () => void; reportState?: HomeHeroReportState }) {
  ...
  const [isRetryLoading, setIsRetryLoading] = React.useState(initialIsRetryLoading);
  ...
  const handleImageLoad = () => {
    if (mode === "original" || isRetryLoading) {
      setMode("original");
      setIsRetryLoading(false);
      setFailedRetryAttempts(0);
    }
  };
  const copyFailedImageLink = async () => {
    const url = getCopyableHomeHeroMediaUrl(failedImageUrl, window.location.origin);
    try {
  ...
  const imageMode = isRetryLoading ? "original" : mode;

  return (
    <>
      <img
        src={getHomeHeroMediaSource(imageMode, retryAttempt)}
        alt={isFallback ? "Abstract neural-network fallback visual — no patient image" : "Abstract non-patient-specific neural-network brain visual"}
        loading="lazy"
        decoding="async"
        onError={handleImageError}
        onLoad={handleImageLoad}
        className="absolute inset-0 h-full w-full object-cover object-center brightness-[0.46] contrast-[1.28] grayscale"
      />
      {isFallback ? (
        <>
          {canRetry ? (
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <button
                    ...
                    aria-busy={isRetryLoading}
                    aria-label={isRetryLoading ? "Retrying the original neural-network image" : "Retry loading the original neural-network image"}
                    className="group absolute right-4 top-4 z-20 inline-flex min-h-9 items-center gap-1.5 rounded-lg border border-[#9be4d5]/70 bg-[#061923]/90 px-3 text-xs font-semibold text-[#def8f0] shadow-lg backdrop-blur-sm transition-[background-color,border-color,box-shadow,transform] duration-200 ease-out hover:-translate-y-0.5 hover:border-[#c8f6ea] hover:bg-[#174a52] hover:shadow-[0_12px_28px_rgba(89,216,193,0.28)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#b8eee2] focus-visible:ring-offset-2 focus-visible:ring-offset-[#061923] active:translate-y-0 active:scale-[0.97] disabled:cursor-wait disabled:opacity-90 disabled:hover:translate-y-0 disabled:hover:bg-[#061923]/90 motion-reduce:transform-none motion-reduce:transition-none"
                  >
                    {isRetryLoading ? <Spinner className="h-3.5 w-3.5 text-[#b8eee2] motion-reduce:animate-none" aria-hidden="true" /> : <RotateCw className="h-3.5 w-3.5 transition-transform duration-200 ease-out group-hover:rotate-[-20deg] group-active:rotate-[100deg] motion-reduce:transform-none motion-reduce:transition-none" />}
                    {isRetryLoading ? RETRY_LOADING_TEXT : "Retry"}
                  </button>
                </TooltipTrigger>
                <TooltipContent side="bottom" sideOffset={10} className="border border-[#8fdccc]/60 bg-[#061923]/95 text-[#e4fbf5] shadow-xl motion-reduce:animate-none">{RETRY_TOOLTIP_TEXT}</TooltipContent>
              </Tooltip>
              <span className="sr-only" role="status" aria-live="polite">{isRetryLoading ? "Retrying the original image. Please wait." : "Original image unavailable. A fallback visual is shown. You can retry loading the original image."}</span>
            </>
          ) : (
