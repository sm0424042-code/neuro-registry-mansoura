...
import { Activity, ArrowRight, BrainCircuit, ClipboardCheck, ClipboardPlus, Eye, FileWarning, HeartPulse, Image as ImageIcon, Microscope, RotateCw, ScanLine, ShieldCheck } from "lucide-react";
...
export function HomeHeroMedia({ initialMode = "original" }: { initialMode?: HomeHeroMediaMode }) {
  ...
  return <><img src={getHomeHeroMediaSource(mode, retryAttempt)} alt={isFallback ? "Abstract neural-network fallback visual — no patient image" : "Abstract non-patient-specific neural-network brain visual"} loading="lazy" decoding="async" onError={() => setMode("fallback")} className="absolute inset-0 h-full w-full object-cover object-center brightness-[0.46] contrast-[1.28] grayscale" />{isFallback ? <><button type="button" onClick={retryOriginalImage} aria-label="Retry loading the original neural-network image" className="absolute right-4 top-4 z-20 inline-flex min-h-9 items-center gap-1.5 rounded-lg border border-[#9be4d5]/70 bg-[#061923]/90 px-3 text-xs font-semibold text-[#def8f0] shadow-lg backdrop-blur-sm transition-colors hover:bg-[#123c44] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#b8eee2] active:scale-[0.97] motion-reduce:transition-none"><RotateCw className="h-3.5 w-3.5" />Retry</button><span className="sr-only" role="status" aria-live="polite">Original image unavailable. A fallback visual is shown. You can retry loading the original image.</span></> : null}</>;
}
