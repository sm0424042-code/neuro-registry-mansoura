...
import { LockKeyhole, MessageSquare, Send, ShieldAlert, UsersRound } from "lucide-react";
...
export default function Messages() {
  ...
  const { data: recipients = [], isLoading: recipientsLoading } = trpc.messages.recipients.useQuery();
  ...
  const { data: thread = [], isLoading: threadLoading } = trpc.messages.thread.useQuery({ recipientUserId: selectedUserId ?? 0 }, { enabled: Boolean(selectedUserId), refetchInterval: 8000 });
  ...
  return <div className="mx-auto max-w-7xl space-y-6 fade-rise">
    ...
    <div className="grid min-h-[620px] overflow-hidden rounded-3xl border border-[#dce9e5] bg-white shadow-sm lg:grid-cols-[310px_minmax(0,1fr)]">
      <aside className="border-b border-[#e7efec] bg-[#f8fbfa] lg:border-b-0 lg:border-r">
        ...
        <ScrollArea className="h-[280px] lg:h-[520px]"><div className="space-y-1 p-3">{recipientsLoading ? <><div className="h-16 animate-pulse rounded-xl bg-slate-100" /><div className="h-16 animate-pulse rounded-xl bg-slate-100" /></> : recipients.length === 0 ? <div className="rounded-xl border border-dashed border-[#dce9e5] p-4 text-sm leading-6 text-slate-500">No other approved users are available yet.</div> : recipients.map(recipient => <button key={recipient.id} onClick={() => setSelectedUserId(recipient.id)} className={`flex w-full items-center gap-3 rounded-xl p-3 text-left transition-colors ${recipient.id === selectedUserId ? "bg-[#dff2ed] text-[#1b615d]" : "hover:bg-white"}`}><Avatar className="h-9 w-9"><AvatarFallback className="bg-[#d7eee8] text-xs text-[#276b65]">{initials(recipient.name)}</AvatarFallback></Avatar><div className="min-w-0"><p className="truncate text-sm font-semibold">{recipient.name || "Approved registry user"}</p><p className="truncate text-xs text-slate-500">{recipient.email || recipient.role}</p></div></button>)}</div></ScrollArea>
      </aside>
