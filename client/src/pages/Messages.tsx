  ...
  return <div className="mx-auto max-w-7xl space-y-6 fade-rise">
    ...
    <div className="grid min-h-[620px] overflow-hidden rounded-3xl border border-[#dce9e5] bg-white shadow-sm lg:grid-cols-[310px_minmax(0,1fr)]">
      ...
      <section className="flex min-h-[620px] flex-col">
        ...
        <div className="border-t border-[#e7efec] bg-white p-4"><div className="mb-3 flex gap-2 rounded-xl border border-[#f0d8bb] bg-[#fff8ef] p-3 text-xs leading-5 text-[#805824]"><ShieldAlert className="mt-0.5 h-4 w-4 shrink-0" /><p>Do not include patient information, names, contact details, addresses, dates of birth, national or medical-record numbers, MUNR Research IDs, report text, or clinical record details in messages.</p></div><form onSubmit={submit} className="flex items-end gap-3"><Textarea value={draft} onChange={event => setDraft(event.target.value)} disabled={!selectedUserId || send.isPending} placeholder={selectedUserId ? "Write a work-only message…" : "Choose an approved colleague first"} className="min-h-[78px] resize-none" maxLength={1000} /><Button type="submit" disabled={!selectedUserId || !draft.trim() || send.isPending} className="h-11 bg-[#125d69] hover:bg-[#0d4b55]"><Send className="mr-2 h-4 w-4" />Send</Button></form></div>
      </section>
    </div>
  </div>;
}
