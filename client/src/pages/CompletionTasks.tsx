...
import { ArrowDownAZ, Bell, ChevronLeft, ChevronRight, ClipboardCheck, Download, History, ListFilter, Search, ShieldCheck, X } from "lucide-react";
...
export type TaskSearchSuggestion = { value: string; label: string };
...
export const safeTaskSearchTerms = ["assigned", "reassigned", "accepted", "completed", "awaiting acceptance"] as const;
export const RECENT_TASK_SEARCHES_STORAGE_KEY = "munr-completion-task-recent-searches-v1";
export const MAX_RECENT_TASK_SEARCHES = 5;
...
export function getTaskSearchSuggestions(searchValue: string, now = new Date()): TaskSearchSuggestion[] {
  const todayUtc = now.toISOString().slice(0, 10);
  const allSuggestions: TaskSearchSuggestion[] = [
    { value: "assigned", label: "Assigned" },
    { value: "reassigned", label: "Reassigned" },
    { value: "accepted", label: "Accepted" },
    { value: "completed", label: "Completed" },
    { value: "awaiting acceptance", label: "Awaiting acceptance" },
    { value: todayUtc, label: `Today’s UTC date: ${todayUtc}` },
  ];
...
export function getActiveTaskSearchSuggestion(suggestions: TaskSearchSuggestion[], activeIndex: number) {
  return Number.isInteger(activeIndex) && activeIndex >= 0 && activeIndex < suggestions.length ? suggestions[activeIndex] : null;
}

export function normaliseRecentTaskSearches(values: unknown): string[] {
  if (!Array.isArray(values)) return [];
  const recentSearches: string[] = [];
  for (const value of values) {
    if (typeof value !== "string") continue;
    const normalised = getTaskSearchValidation(value).normalized;
    if (normalised && !recentSearches.includes(normalised)) recentSearches.push(normalised);
    if (recentSearches.length === MAX_RECENT_TASK_SEARCHES) break;
  }
  return recentSearches;
}

export function addRecentTaskSearch(recentSearches: string[], searchValue: string): string[] {
  const search = getTaskSearchValidation(searchValue).normalized;
  return search ? normaliseRecentTaskSearches([search, ...recentSearches]) : normaliseRecentTaskSearches(recentSearches);
}

export function readRecentTaskSearches(): string[] {
  if (typeof window === "undefined") return [];
  try {
    return normaliseRecentTaskSearches(JSON.parse(window.localStorage.getItem(RECENT_TASK_SEARCHES_STORAGE_KEY) ?? "[]"));
  } catch {
    return [];
  }
}

export function persistRecentTaskSearches(recentSearches: string[]) {
  if (typeof window === "undefined") return;
  try {
    window.localStorage.setItem(RECENT_TASK_SEARCHES_STORAGE_KEY, JSON.stringify(normaliseRecentTaskSearches(recentSearches)));
  } catch {
    // Local preferences are optional; a browser storage error must not affect protected task access.
  }
}

export function getClearedTaskSearchState() {
  return { taskSearch: "", currentPage: 1 };
}

export function getTaskSearchSuggestionPresentation(isRecentSearch: boolean) {
  return isRecentSearch ? { accessibleLabel: "Recent safe search", showsHistoryIcon: true } : { accessibleLabel: "Safe search suggestion", showsHistoryIcon: false };
}
...
export function getTaskStatusLabel(status: string) {
  return status === "reassigned" ? "Reassigned — accept again" : status.charAt(0).toUpperCase() + status.slice(1);
}
...
function TaskSearchBox({ value, recentSearches, disabled, error, onValueChange }: { value: string; recentSearches: string[]; disabled: boolean; error: string | null; onValueChange: (value: string) => void }) {
  const [open, setOpen] = useState(false);
  const [activeIndex, setActiveIndex] = useState(-1);
  const searchInputRef = useRef<HTMLInputElement>(null);
  const suggestions = useMemo(() => value.trim() ? getTaskSearchSuggestions(value) : recentSearches.map(search => ({ value: search, label: `Recent search: ${search}` })), [value, recentSearches]);
  const isRecentSearchList = !value.trim();
  const listboxId = "task-search-suggestions";
  const chooseSuggestion = (suggestion: TaskSearchSuggestion) => {
  ...
  const onKeyDown = (event: KeyboardEvent<HTMLInputElement>) => {
    if (event.key === "ArrowDown" || event.key === "ArrowUp") {
      ...
      setOpen(true);
      setActiveIndex(current => getNextTaskSearchSuggestionIndex(current, event.key as "ArrowDown" | "ArrowUp", suggestions.length));
    } else if (event.key === "Enter" && open && getActiveTaskSearchSuggestion(suggestions, activeIndex)) {
      event.preventDefault();
      chooseSuggestion(getActiveTaskSearchSuggestion(suggestions, activeIndex)!);
  ...
  const suggestionPresentation = getTaskSearchSuggestionPresentation(isRecentSearchList);
  return <div className="relative"><label htmlFor="task-quick-search" className="mb-1.5 flex items-center gap-1.5 text-xs font-medium text-[#45616a]"><Search className="h-3.5 w-3.5" />Quick search</label><div className="relative"><Input ref={searchInputRef} id="task-quick-search" className={value ? "pr-10" : undefined} value={value} disabled={disabled} onChange={event => { onValueChange(event.target.value); setOpen(true); setActiveIndex(-1); }} onFocus={() => setOpen(true)} onBlur={() => window.setTimeout(() => setOpen(false), 120)} onKeyDown={onKeyDown} placeholder="Status or UTC date" role="combobox" aria-autocomplete="list" aria-keyshortcuts="ArrowUp ArrowDown Enter Escape" aria-expanded={open && suggestions.length > 0} aria-controls={listboxId} aria-activedescendant={activeIndex >= 0 ? `${listboxId}-${activeIndex}` : undefined} aria-describedby="task-search-help" aria-invalid={Boolean(error)} />{value && <button type="button" className="absolute right-1.5 top-1/2 -translate-y-1/2 rounded-md p-1.5 text-slate-500 transition-colors hover:bg-slate-100 hover:text-[#245961] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#246c6e] disabled:pointer-events-none disabled:opacity-50" aria-label="Clear task search and restore the full filtered list" title="Clear search" disabled={disabled} onMouseDown={event => event.preventDefault()} onClick={clearSearch}><X className="h-4 w-4" aria-hidden="true" /></button>}</div>{open && suggestions.length > 0 && <div id={listboxId} role="listbox" aria-label={isRecentSearchList ? "Recent safe task searches" : "Safe task search suggestions"} className="absolute z-20 mt-1 w-full overflow-hidden rounded-lg border border-[#cfe1dc] bg-white p-1 shadow-lg"><p className="px-2 py-1 text-[11px] font-medium text-slate-500">{isRecentSearchList ? "Recent safe searches on this device" : "Safe search suggestions"}</p>{suggestions.map((suggestion, index) => <button key={suggestion.value} id={`${listboxId}-${index}`} type="button" role="option" aria-label={`${suggestionPresentation.accessibleLabel}: ${suggestion.label}`} aria-selected={index === activeIndex} className={`flex w-full items-center gap-2 rounded-md px-2.5 py-2 text-left text-sm transition-colors ${index === activeIndex ? "bg-[#e9f6f2] text-[#18565f]" : "text-[#36535a] hover:bg-[#f4faf8]"}`} onMouseDown={event => event.preventDefault()} onClick={() => chooseSuggestion(suggestion)}>{suggestionPresentation.showsHistoryIcon ? <History className="h-3.5 w-3.5 shrink-0 text-[#2f746b]" aria-hidden="true" /> : null}<span>{splitTaskSearchSuggestionHighlight(suggestion.label, value).map((segment, segmentIndex) => segment.matched ? <mark key={`${suggestion.value}-${segmentIndex}`} className="rounded-sm bg-amber-100 px-0.5 font-semibold text-[#704b05]">{segment.text}</mark> : <span key={`${suggestion.value}-${segmentIndex}`}>{segment.text}</span>)}</span></button>)}</div>}</div>;
}
...
export default function CompletionTasks() {
  ...
  const [recentTaskSearches, setRecentTaskSearches] = useState<string[]>(readRecentTaskSearches);
  ...
  useEffect(() => { if (taskSearchValidation.normalized) setRecentTaskSearches(current => addRecentTaskSearch(current, taskSearchValidation.normalized!)); }, [taskSearchValidation.normalized]);
  useEffect(() => { persistRecentTaskSearches(recentTaskSearches); }, [recentTaskSearches]);
