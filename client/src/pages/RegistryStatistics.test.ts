...
import { buildComparisonSummary, buildCustomStatisticSummary, createAggregateStatisticsCsv, createCustomStatisticCsv, getComparisonSignal, getCustomStatisticInsight, getCustomStatisticOptions, getCustomStatisticViewData, getPercentagePointDifferenceLabel, getStatisticDifferenceLabel, getStatisticShare, getStatisticsChartPngFileName, getStatisticsDateRangeLabel, normalizeSavedCustomStatisticQueries, normalizeStatisticsViewPreference, statisticLabel, toAggregateChartData, toAggregateComparisonChartData, toComparisonReportRows, toCohortIndicatorDistributionComparison, type AggregateStatistics } from "./RegistryStatistics";
...
describe("RegistryStatistics", () => {
  ...
  it("builds allowed chart views, bounded saved-query definitions, and a non-causal smart interpretation", () => {
    ...
    expect(getCustomStatisticInsight(strokeShare)).toMatchObject({ currentShare: 60, comparisonShare: 43, remainingShare: 40 });
    expect(getCustomStatisticInsight(strokeShare).note).toContain("does not establish");
    expect(normalizeSavedCustomStatisticQueries([{ id: "saved-stroke-share", family: "cohort", selectedKey: "cohort:stroke", view: "comparison" }, { id: "unsafe", family: "free_text", selectedKey: "patient:name", view: "raw" }])).toEqual([{ id: "saved-stroke-share", family: "cohort", selectedKey: "cohort:stroke", view: "comparison" }]);
    expect(JSON.stringify(getCustomStatisticInsight(strokeShare))).not.toMatch(/MUNR|research.?id|patient|diagnosis|clinical narrative|email|recorded by/i);
  });
});
