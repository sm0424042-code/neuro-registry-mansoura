...
import { ArrowUpRight, Check, Download, FileSpreadsheet, KeyRound, Mail, Search, ShieldAlert, ShieldCheck, UsersRound, X } from "lucide-react";
...
export const UPGRADE_ACCESS_LABEL = "Upgrade access";
export const UPGRADE_ADMIN_LABEL = "Upgrade to administrator";
export const OAUTH_ONBOARDING_GUIDANCE = "Sign in with your OAuth account. The provider supplies the account name and email when available; create and protect any strong password with the provider, not in this registry.";
...
export default function AccessManagement() {
  ...
  return <div className="mx-auto max-w-7xl space-y-6 fade-rise">
    ...
    <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      <Card className="border-[#dce9e5] shadow-sm"><CardContent className="p-5"><span className="grid h-9 w-9 place-items-center rounded-xl bg-[#edf7f4] text-[#2d716a]"><KeyRound className="h-4 w-4" /></span><p className="mt-4 text-xs font-bold tracking-[0.12em] text-[#3d7a72]">1. SECURE SIGN-IN</p><p className="mt-2 text-sm leading-6 text-slate-600">{OAUTH_ONBOARDING_GUIDANCE}</p></CardContent></Card>
      <Card className="border-[#dce9e5] shadow-sm"><CardContent className="p-5"><span className="grid h-9 w-9 place-items-center rounded-xl bg-[#edf7f4] text-[#2d716a]"><UsersRound className="h-4 w-4" /></span><p className="mt-4 text-xs font-bold tracking-[0.12em] text-[#3d7a72]">2. APPLICANT DETAILS</p><p className="mt-2 text-sm leading-6 text-slate-600">After secure sign-in, the account appears as pending for manual review. The approver reviews the OAuth display name, provider-linked email when available, linked-identity state, requested role, and protocol authorisation. No phone number or national ID is collected.</p></CardContent></Card>
