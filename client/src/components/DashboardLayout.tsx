...
export const DEFAULT_ACCOUNT_DISPLAY_NAME = "Abdelrahman Ibrahim Rashad";

export function getSidebarAccountDisplayName(name: string | null | undefined) {
  return name?.trim() || DEFAULT_ACCOUNT_DISPLAY_NAME;
}
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  if (user.accessStatus !== "approved" || user.removedAt) return <AccessPending status={user.accessStatus === "approved" ? "suspended" : user.accessStatus} logout={logout} />;
...
function DashboardLayoutContent({ children, setSidebarWidth }: { children: React.ReactNode; setSidebarWidth: (width: number) => void }) {
  ...
  const taskNotifications = trpc.completionTasks.notifications.useQuery(undefined, { enabled: user?.role === "admin" });
  const unreadTaskNotifications = taskNotifications.data?.filter(notification => !notification.readAt).length ?? 0;
