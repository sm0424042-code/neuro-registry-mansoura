  ...
  return (
    <div className="clinical-grid min-h-screen bg-[#edf3f1] p-5 flex items-center justify-center">
      <div className="fade-rise w-full max-w-5xl overflow-hidden rounded-[1.7rem] border border-[#dce8e4] bg-white shadow-[0_28px_80px_rgba(17,43,56,0.14)]">
        ...
        <div className="grid gap-8 p-8 lg:grid-cols-[0.8fr_1.2fr]"><div><p className="text-[10px] font-bold tracking-[0.15em] text-[#33766e]">SECURE ACCESS</p><h1 className="mt-2 font-display text-3xl text-[#172b38]">Authorised access only</h1><p className="mt-3 text-sm leading-6 text-slate-500">Sign in to request or use approved access. Patient records are available only to authorised research personnel.</p><Button onClick={() => startLogin()} className="mt-7 w-full bg-[#125d69] hover:bg-[#0d4b55]">Sign in securely</Button></div><div className="rounded-2xl border border-[#dce9e5] bg-[#f5faf8] p-5"><div className="flex items-center gap-2"><ClipboardCheck className="h-4 w-4 text-[#2f746b]" /><p className="text-[11px] font-bold tracking-[0.14em] text-[#397a72]">LATEST CLINICAL WORKFLOW ADDED</p></div><h2 className="mt-2 font-display text-xl text-[#1c3d47]">New structured fields are ready after sign-in.</h2><div className="mt-4 grid gap-3 sm:grid-cols-2"><ReleaseSummary icon={HeartPulse} title="Stroke" detail="Complications, NIHSS / mRS evaluation, and discharge treatment." /><ReleaseSummary icon={BrainCircuit} title="Multiple Sclerosis" detail="TB screening, MSFC, DMTs, and dated dose adherence." /><ReleaseSummary icon={Microscope} title="CIDP & neuropathy" detail="Typical CIDP, MADSAM, mononeuritis multiplex, vasculitic pathway, and immune therapy." /><ReleaseSummary icon={Eye} title="Neuro-ophthalmology" detail="NMOSD, MOGAD, GCA, AQP4/MOG profile, and linked systemic disease." /></div><p className="mt-4 text-xs leading-5 text-[#587579]">This release summary contains no patient information. Registry data remains hidden until access is approved.</p></div></div>
      </div>
    </div>
  );
}
