...
export const appRouter = router({
  ...
  completionTasks: router({
    ...
    exportCsv: approvedProcedure.input(completionTaskExportInputSchema).mutation(async ({ ctx, input }) => {
      const rows = await db.listRecordCompletionTaskExportRows(ctx.user.id, ctx.user.role === "admin", resolveCompletionTaskExportInput(input));
      return { filename: `completion-tasks-${new Date().toISOString().slice(0, 10)}.csv`, csv: createCompletionTaskCsv(rows), exportedRows: rows.length, isTruncated: rows.length === 10_000 };
    }),
